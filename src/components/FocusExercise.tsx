...
import { ArrowRight, Brain, Check, RotateCw, Target } from 'lucide-react'
...
const FocusExercise = ({
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setTargetPosition({
        x: Math.min(
          Math.max(targetPosition.x + Math.random() * 10 - 5, 10),
          90
        ),
        y: Math.min(
          Math.max(targetPosition.y + Math.random() * 10 - 5, 10),
          90
        ),
      })
    ...
    <div className="w-full">
      ...
      <div className="mb-4">
        <div className="flex justify-between text-xs text-gray-500 mb-1">
          <span>Progress</span>
          <span>
            {exerciseIndex + 1}/{EXERCISE_TYPES.length}
          </span>
        </div>
        <Progress
          value={
            ((exerciseIndex + (isCompleted ? 1 : 0)) / EXERCISE_TYPES.length) *
            100
          }
        />
      </div>
      ...
      <div className="bg-blue-50 rounded-lg p-3 text-sm text-blue-700 mb-2">
        {currentExerciseData.id === 'sustained' && (
          <p>
            Follow the blue circle with your eyes, try to ignore the gray
            distractions.
          </p>
        )}
        {currentExerciseData.id === 'selective' && (
          <p>
            Focus on finding patterns in the movements while maintaining
            concentration.
          </p>
        )}
        {currentExerciseData.id === 'divided' && (
          <p>
            Mentally track both the position and the movement pattern
            simultaneously.
          </p>
        )}
      </div>
